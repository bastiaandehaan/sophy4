...
SYMBOLS = [
    "GER40.cash",  # 112 trades/year achieved
...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self.trading_modes = {
            ...
            "aggressive": TradingModeConfig(
                ...
                target_trades_per_year=1200,
                parameters={
                    # ULTRA AGGRESSIVE PARAMETERS
                    "use_htf_confirmation": False,
                    "stress_threshold": 20.0,            # ULTRA HIGH
    ...
    def set_mode(self, mode: str) -> bool:
        ...
        logger.info(f"Target frequency: {mode_config.target_trades_per_year} trades/year")
        logger.info(f"HTF Confirmation: {mode_config.parameters.get('use_htf_confirmation')}")
        logger.info(f"Risk per trade: {mode_config.parameters.get('risk_per_trade'):.1%}")

        return True

    def get_strategy_params(self, strategy_name: str, symbol: str = None, **overrides) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Generated {strategy_name} params for {params['symbol']} in {self.current_mode.upper()} mode")
    ...
    def get_portfolio_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expected_trades_per_symbol": mode_config.target_trades_per_year // len(SYMBOLS),
            "initial_capital_per_symbol": 20000.0,
    ...
    def validate_mode_performance(self, trades_per_year: float, mode: str = None) -> Dict[str, Any]:
        ...
        return {
            "valid": trades_per_year >= min_acceptable,
            "status": status,
            "message": message,
...
def fetch_historical_data(symbol: str, timeframe: str = "H1", days: int = 365) -> Optional[pd.DataFrame]:
    """
    ...
    try:
        ...
        logger.info(f"Fetching data for {symbol}, timeframe: {timeframe}, from {pd.Timestamp.now() - pd.Timedelta(days=days)} to {pd.Timestamp.now()}")

        # Mock data generation for testing - replace with actual MT5 integration
        import numpy as np

        # Generate realistic price data
        periods = int(days * 24 / {"M15": 0.25, "M30": 0.5, "H1": 1, "H4": 4, "D1": 24}[timeframe])
        ...
        # Generate OHLC data
        base_price = 18000 if "GER40" in symbol else (2000 if "XAUUSD" in symbol else 1.1)
        ...
        highs = np.maximum(opens, closes) * (1 + np.abs(np.random.normal(0, 0.003, periods)))
        lows = np.minimum(opens, closes) * (1 - np.abs(np.random.normal(0, 0.003, periods)))
        ...
        logger.info(f"Historical data loaded: {len(df)} rows, columns: {list(df.columns)}")
...
def get_strategy_params(strategy_name: str, symbol: str = None, **overrides) -> Dict[str, Any]:
...
logger.info(f"Expected portfolio frequency: {config_manager.trading_modes[config_manager.current_mode].target_trades_per_year} trades/year")